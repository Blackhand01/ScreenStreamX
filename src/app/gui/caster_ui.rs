...
fn start_broadcast(app: &mut MyApp) {
    ...
    create_recording_directory("recording");

    thread::spawn(move || {
        start_broadcast_thread(recording_flag_clone, rx, capture_area, width, height);
    });
}
...
/// Avvia il thread per la trasmissione dello schermo
fn start_broadcast_thread(
    ...
    let mut child = Command::new("ffmpeg")
        .args(&[
            ...
            "-y", "recording/broadcast_video.mp4" // Nome del file di output con percorso
        ])
        .stdin(Stdio::piped())
        .spawn()
    ...
    loop {
        ...
        broadcast_frame(&mut screen_capturer, &mut out, width, height);
    }

    println!("Broadcast thread exiting");
}

/// Esegui la trasmissione di un singolo frame
fn broadcast_frame(
